...
- Run state lives under **`.agent-state/runs/<date>/<slug>/<version>/manifest.json`** (the versioned layout `RunDir` resolves first; use `vibe-agent migrate docs-tmp` to move an old workspace-root tmp/ tree used only when versioned resolution fails). Do not write it directly; use **`checkpoint`** and **`run`** packages.
...
3. **Record evidence** under **`.agent-state/runs/<date>/<slug>/<version>/browser/`** (the versioned layout; after migrate, only `.agent-state/runs/` holds browser evidence) at the workspace root (gitignored). For each session include:
