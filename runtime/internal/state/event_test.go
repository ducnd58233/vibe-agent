...
func TestAppendEventNumbersSequentiallyFromOne(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		if _, err := AppendEvent(path, Event{Type: "node_entered", Node: "build", At: fixedTime()}); err != nil {
...
func TestAppendEventWritesOneJSONObjectPerLine(t *testing.T) {
	...
	if _, err := AppendEvent(path, Event{Type: "verifier_ran", Node: "test", Payload: payload, At: fixedTime()}); err != nil {
		t.Fatalf("AppendEvent: %v", err)
	}
	if _, err := AppendEvent(path, Event{Type: "node_entered", Node: "e2e", At: fixedTime()}); err != nil {
		t.Fatalf("AppendEvent: %v", err)
	}
...
func TestAppendEventRejectsAnEmptyType(t *testing.T) {
	path := EventLogPath(t.TempDir(), "slug")
	if _, err := AppendEvent(path, Event{At: fixedTime()}); err == nil {
...
func TestAppendEventReturnsTheStoredEvent(t *testing.T) {
	path := EventLogPath(t.TempDir(), "slug")
	stored, err := AppendEvent(path, Event{Type: "node_entered", Node: "build", At: fixedTime()})
	if err != nil {
		t.Fatalf("AppendEvent: %v", err)
	}
...
func TestAppendEventDefaultsMissingTimestamp(t *testing.T) {
	...
	stored, err := AppendEvent(path, Event{Type: "node_entered"})
	if err != nil {
		t.Fatalf("AppendEvent: %v", err)
	}
