...
- **Scope boundary:** This repository is **not** a product-domain codebase. Domain-specific behavior belongs in each consuming repo via its local `AGENTS.md`. It does ship toolkit infrastructure code: the validation scripts under [`scripts/`](scripts) and the runtime control plane under [`runtime/`](runtime). The runtime enforces the outer delivery loop (graph transitions, run state, verification evidence); it never replaces a coding agent's own model and tool loop. **It is required by the delivery pipeline** (`/goal`, `/build`, `/test`, `/review`, `/ship`), which refuses to run without it, and optional for reference assets. Canonical rules: [`commands/goal.md`](.ai-agents/commands/goal.md) section "Runtime is required".
...
- **Stack detection (MUST):** do not assume a global stack. Before applying any skill's guidance, inspect the workspace manifests and existing patterns, then open [`stack-profiles/ROUTER.md`](.ai-agents/stack-profiles/ROUTER.md) when present, select every profile row applicable to the task, and read those files. Product and domain expectations come from the workspace-root `AGENTS.md`. This rule is stated once here; individual skills do not repeat it.
- **Security first (MUST):** apply [`secure-by-default`](.ai-agents/skills/secure-by-default/SKILL.md) to any work touching auth, user data, logging, error handling, configuration, or a client surface. Sensitive values must not reach a channel the end user, an outside developer, or an attacker can read: client bundles, browser or device storage, consoles and device logs, rendered UI and the DOM, API responses, server logs and telemetry, build artifacts, or `tmp/` evidence records. Redact at the boundary so a later caller inherits it, and give errors two shapes: a stable code and a safe message for the caller, full detail server-side behind a request ID. This is a **write-time** constraint, not a review-time one; `/ship` sees only what already exists. Channel detail: [`sensitive-data-exposure.md`](.ai-agents/references/sensitive-data-exposure.md). The deterministic sensor is [`sensitive-data-guard.py`](.ai-agents/hooks/sensitive-data-guard.py); a clean run is a floor, not evidence.
